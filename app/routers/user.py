...
from fastapi import APIRouter, Depends, HTTPException, Path as FastAPI_Path, UploadFile, File
...
def upload_profile_picture(
    ...
    pictures_path = current_dir[: current_dir.rindex("/")] + "/static/images/profile-pictures"
...
def delete_user(user_id: int = FastAPI_Path(...), db: Session = Depends(database.get_db),
                current_user=Depends(oauth2.get_current_user)):
    return user_service.delete_user(user_id, db)


@router.get("/{user_id}/cars", response_model=List[CarDisplay])
def read_cars_by_user(user_id: int = FastAPI_Path(...), db: Session = Depends(database.get_db)):
